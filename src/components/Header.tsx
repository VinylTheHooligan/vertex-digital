...
export default function Header() {
    ...
    return (
        <header className="sticky top-0 z-50 grid grid-cols-3 grid-rows-1 backdrop-blur-xs backdrop-brightness-85 py-4 mb-10">
            <Link aria-label="Logo et lien vers la page d'accueil" className="col-start-2 flex justify-center" href="/">
            ...
            <label aria-label="Bouton bascule de thème" className="flex items-center justify-end mx-5" suppressHydrationWarning>
                    <div className="relative rounded-full duration-300 w-16 h-8 ring-2 ring-foreground">
                        <div className="relative inline-flex cursor-pointer h-8 w-16 select-none">
                            <input
                              type="checkbox"
                              className="sr-only"
                              checked={isDark}
                              onChange={toggle}
                            />
                            <div suppressHydrationWarning className={`absolute left-1 top-1 h-6 w-6 rounded-full bg-foreground duration-300 flex items-center justify-center ${isDark ? "translate-x-8" : "translate-x-0"}`}>
                                <svg
                                ...
                                <svg suppressHydrationWarning viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className={`absolute w-4 h-4 text-background transition-opacity duration-300 ${isDark ? "opacity-100" : "opacity-0"}`}>
